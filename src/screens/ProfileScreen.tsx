...
export function ProfileScreen() {
  ...
  const handleLanguageChange = () => {
    ...
    void i18n.changeLanguage(SUPPORTED_LANGUAGES[nextIndex].code);
  };
  ...
  return (
    ...
    >
      ...
      <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.surfaceBorder }]}>
        <Text style={[styles.userName, { color: theme.colors.text }]}>{user?.name || user?.username}</Text>
        <Text style={[styles.userEmail, { color: theme.colors.textMuted }]}>{user?.email}</Text>
      </View>

      <View style={[styles.section, { backgroundColor: theme.colors.surface, borderColor: theme.colors.surfaceBorder }]}>
        <Text style={[styles.sectionTitle, { color: theme.colors.textMuted }]}>{t("profile.settings")}</Text>
        <TouchableOpacity style={styles.row} onPress={toggleTheme}>
          <Ionicons name={mode === "dark" ? "moon" : "sunny"} size={20} color={theme.colors.icon} />
          <Text style={[styles.rowLabel, { color: theme.colors.text }]}>{t("profile.theme")}</Text>
        ...
        <TouchableOpacity style={styles.row} onPress={handleLanguageChange}>
          ...
          <Text style={[styles.rowValue, { color: theme.colors.textMuted }]}>{currentLang?.label}</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  ...
  section: { borderRadius: 12, borderWidth: 1, padding: 16, marginBottom: 24 },
  userName: { fontSize: 20, fontWeight: "700" },
  userEmail: { marginTop: 4, fontSize: 14 },
  sectionTitle: { fontSize: 12, fontWeight: "600", textTransform: "uppercase", letterSpacing: 1, marginBottom: 16 },
  row: { flexDirection: "row", alignItems: "center", paddingVertical: 12 },
  rowLabel: { flex: 1, fontSize: 16, marginLeft: 12 },
  rowValue: { fontSize: 14 },
  divider: { height: 1, marginVertical: 4 },
  logoutButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", borderRadius: 12, borderWidth: 1, padding: 16, gap: 8 },
